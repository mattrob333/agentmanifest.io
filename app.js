...
const softwareLogos = {
  "Microsoft": "MS", "Microsoft Entra": "ME", "Google": "G", "Google Workspace": "GW", "Okta": "OK",
  "Workday": "WD", "BambooHR": "BH", "ADP": "ADP", "Paylocity": "PL", "Rippling": "RP", "Gusto": "GU",
  "Salesforce": "SF", "ServiceNow": "SN", "Snowflake": "SF", "NetSuite": "NS", "GitHub": "GH",
  "Microsoft 365": "365", "Slack": "SL", "SAP": "SAP", "HubSpot": "HS", "Marketo": "MK"
};
...
function emptyState(){return {id:crypto.randomUUID(),active:"company",demoLoaded:false,generated:null,selected:null,company:{},humans:[],agents:[],policy:{},goals:{}};}
let state = JSON.parse(localStorage.getItem("pedigreeWorkspace") || "null") || emptyState();
let saveTimer;
...
function render(){
  ...
  app.innerHTML = `<div class="shell">
    ...
    <main class="main"><div class="content">${sectionHtml()}${state.generated?downloadHtml():""}<div class="saveState">Workspace ${state.id} - autosaved locally</div></div></main></div>${state.selected?inspectorHtml():""}<div class="toast" id="toast"></div>`;
}
...
function hasData(){return state.demoLoaded||state.humans.length||state.agents.length||Object.keys(state.company).length||Object.keys(state.policy).length||Object.keys(state.goals).length;}
...
function cards(section,key,label,opts){return `<div class="field full"><label>${label}</label><div class="logoGrid">${opts.map(o=>`<button class="logoCard ${(state[section][key]||"")===o?"selected":""}" onclick="update('${section}','${key}','${o}')"><span>${softwareLogos[o]||o.slice(0,2).toUpperCase()}</span><strong>${o}</strong></button>`).join("")}</div></div>`;}
function chipSelect(section,key,label,opts){return `<div class="field full"><label>${label}</label><div class="checks">${opts.map(o=>`<button class="choiceChip ${(state[section][key]||"")===o?"selected":""}" onclick="update('${section}','${key}','${o}')">${o}</button>`).join("")}</div></div>`;}
function sectionHtml(){
  if(state.active==="company") return `<h1>Company Profile</h1><p class="lede">Capture the company baseline Pedigree needs for governance, risk, and dashboard context.</p><div class="panel grid">${input("company","company_name","Legal company name")}${input("company","operating_name","Operating name")}${chipSelect("company","industry","Industry",options.industry)}${chipSelect("company","employee_count_band","Employee count",options.employee_count_band)}${chipSelect("company","annual_revenue_band","Annual revenue",options.annual_revenue_band)}${input("company","headquarters_country","Headquarters country")}${input("company","headquarters_city","Headquarters city")}${cards("company","primary_it_environment","Primary IT environment",options.primary_it_environment)}${cards("company","identity_provider","Identity provider",options.identity_provider)}${cards("company","hris_system","HRIS system",options.hris_system)}${checks("company","regulatory_exposure","Regulatory exposure",options.regulatory_exposure)}</div>`;
  ...
  if(state.active==="policy") return `<h1>Policy & Approval Rules</h1><p class="lede">Capture the current governance posture, even where the policy is informal or still in progress.</p><div class="panel grid">${chipSelect("policy","agents_must_have_human_sponsor","Human sponsor policy",options.simple.agents_must_have_human_sponsor)}${chipSelect("policy","external_email_approval","External email approval",options.simple.external_email_approval)}${chipSelect("policy","financial_action_approval","Financial action approval",options.simple.financial_action_approval)}${input("policy","financial_threshold","Financial threshold","number")}${text("policy","forbidden_agent_types","Forbidden agent types")}${select("policy","ai_tool_approver_email","AI tool approver",state.humans.map(h=>h.email))}${chipSelect("policy","audit_cycle","Audit cycle",options.simple.audit_cycle)}${chipSelect("policy","has_formal_ai_policy","Formal AI policy",options.simple.policy)}${chipSelect("policy","ai_risk_register_exists","AI risk register",options.simple.yesNo)}${chipSelect("policy","recent_ai_incident","Recent AI incident",options.simple.incident)}</div>`;
  return `<h1>Goals & Context</h1><p class="lede">Give the manifest enough executive context to explain why the deployment matters now.</p><div class="panel grid">${checks("goals","deployment_reasons","Deployment reasons",options.simple.deployment_reasons)}${chipSelect("goals","upcoming_event","Upcoming event",options.simple.upcoming_event)}${input("goals","upcoming_event_date","Upcoming event date","date")}${text("goals","success_90_days","Success in 90 days")}${text("goals","success_12_months","Success in 12 months")}${text("goals","specific_concerns","Specific concerns")}</div>`;
}
...
function downloadTemplate(type){const body=type==="humans"?"email,full_name,title,department,manager_email,employee_id,office_location,employment_status,job_level\nalex.owner@example.com,Alex Owner,CEO,Executive,,EMP-001,New York,active,E8\nsam.manager@example.com,Sam Manager,Director of Operations,Operations,alex.owner@example.com,EMP-002,Chicago,active,M4\n":"agent_name,platform,owner_email,department,purpose,systems_touched,actions_capable,it_security_approved,last_known_active,risk_self_assessment\nInvoice Review Agent,Microsoft Copilot Studio,sam.manager@example.com,Operations,Reviews incoming invoices for routing,Microsoft 365|NetSuite,read_only|write_internal,unsure,2026-05-01,medium\n"; download(`${type}_template.csv`,body,"text/csv;charset=utf-8");}
...
function confirmReset(){app.insertAdjacentHTML("beforeend",`<div class="modalBackdrop"><div class="modal"><h2>Clear workspace data?</h2><p class="lede">This removes demo data and gives you a clean workspace for real customer onboarding.</p><div class="row"><button class="btn demo" onclick="clearData()">Clear Data</button><button class="btn secondary" onclick="render()">Cancel</button></div></div></div>`);}
function clearData(){state={...emptyState(),active:state.active}; persist(); render(); toast("Workspace cleared");}
function departments(){return [...new Set(state.humans.map(h=>h.department).filter(Boolean))];}
function inspectRow(type,i){state.selected={type,index:i}; persist(); render();}
function inspectorHtml(){const r=state[state.selected.type][state.selected.index]; if(!r)return ""; const owner=state.humans.find(h=>h.email===r.owner_email); const reports=state.humans.filter(h=>h.manager_email===r.email).length; const owned=state.agents.filter(a=>a.owner_email===r.email).length; return `<div class="modalBackdrop"><div class="modal inspector"><div class="row between"><h2>${esc(r.full_name||r.agent_name)}</h2><button class="btn secondary" onclick="state.selected=null;persist();render()">Close</button></div><div class="metaGrid">${Object.entries(r).map(([k,v])=>`<div><label>${k}</label><p>${esc(Array.isArray(v)?v.join(", "):v||"empty")}</p></div>`).join("")}${state.selected.type==="humans"?`<div><label>direct reports</label><p>${reports}</p></div><div><label>owned agents</label><p>${owned}</p></div><div><label>permission tier</label><p>${permissionTier(r)}</p></div>`:`<div><label>human owner</label><p>${owner?owner.full_name:"Orphaned"}</p></div><div><label>risk reason</label><p>${riskReason(r)}</p></div>`}</div></div></div>`;}
function permissionTier(h){if((h.job_level||"").startsWith("E"))return "Executive / broad administrative visibility"; if((h.job_level||"").startsWith("M"))return "Manager / team workflow access"; return "Contributor / scoped application access";}
function riskReason(a){const reasons=[]; if(!a.owner_email)reasons.push("No owner"); if((a.risk_self_assessment||"")==="high")reasons.push("High self-assessment"); if(intersects(a.actions_capable||[],["move_money","delete_records","modify_permissions"]))reasons.push("Sensitive action capability"); if(intersects(a.systems_touched||[],["NetSuite","Snowflake","SAP","Workday"]))reasons.push("Sensitive system touched"); return reasons.join("; ")||"No elevated risk marker";}
function miniViz(type){const rows=state[type]; if(!rows.length)return `<p class="hint">Import or add records to populate the onboarding visualization.</p>`; const groups=Object.entries(rows.reduce((a,r)=>{const k=r.department||"Unassigned"; a[k]=(a[k]||0)+1; return a;},{})); return `<div class="vizHeader"><label>${type==="humans"?"Org intake visualization":"Agent inventory visualization"}</label><span>${rows.length} records</span></div><div class="vizGrid">${groups.map(([k,v])=>`<button class="vizNode"><strong>${esc(k)}</strong><span>${v} ${type==="humans"?"people":"agents"}</span></button>`).join("")}</div>`;}
...
function download(name,body,type){const a=document.createElement("a"); a.href=URL.createObjectURL(new Blob([body],{type})); a.download=name; a.style.display="none"; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href); a.remove();},250);}
function downloadBlob(name,blob,type){const a=document.createElement("a"); a.href=URL.createObjectURL(blob instanceof Blob?blob:new Blob([blob],{type})); a.download=name; a.style.display="none"; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href); a.remove();},250);}
